...
! function (a) {
    ...
    function k() {
        ...
        f.device = i, f.getVoices = function () {
            ...
            var b = function (a) {
                a.hasOwnProperty("indexes") ? k.push(a) : (console.error("The following command doesn't provide any index to execute :"), console.dir(a))
            };
        ...
        var n = function (b, e, i, j) {
                ...
                } else console.warn("No voice was selected during the initialization probably because there were no voices available. Initialize artyom after the onload event of the window.");
                1 == e && k.addEventListener("start", function () {
                    c.speaking = !0, f.debug("Event reached : " + g.SPEECH_SYNTHESIS_START), m(g.SPEECH_SYNTHESIS_START), j && "function" == typeof j.onStart && j.onStart.call(k)
                }), e >= i && k.addEventListener("end", function () {
                    c.speaking = !1, f.debug("Event reached : " + g.SPEECH_SYNTHESIS_END), m(g.SPEECH_SYNTHESIS_END), j && "function" == typeof j.onEnd && j.onEnd.call(k)
        ...
        var p = function (a, d) {
            var h, i;
            f.device.isMobile ? (b.continuous = !1, b.interimResults = !1, b.maxAlternatives = 1) : (b.continuous = !0, b.interimResults = !0), b.lang = c.lang, b.onstart = function () {
            ...
            if ("normal" == c.mode && (j = function (a) {
                    ...
                    for (var e = a.resultIndex; e < d; ++e) {
                        ...
                        if (a.results[e].isFinal) {
                            var i = q(h.trim());
                            if ("function" == typeof c.helpers.redirectRecognizedTextOutput && c.helpers.redirectRecognizedTextOutput(h, !0), i !== !1 && 1 == c.recognizing) {
                                f.debug("<< Executing Matching Recognition in normal mode >>", "info"), b.stop(), c.recognizing = !1, i.wildcard ? i.objeto.action(i.indice, i.wildcard.item, i.wildcard.full) : i.objeto.action(i.indice);
                                break
                            }
                        } else {
                            if ("function" == typeof c.helpers.redirectRecognizedTextOutput && c.helpers.redirectRecognizedTextOutput(h, !1), "string" == typeof c.executionKeyword && h.indexOf(c.executionKeyword) != -1) {
                                var i = q(h.replace(c.executionKeyword, "").trim());
                                if (i !== !1 && 1 == c.recognizing) {
                                    f.debug("<< Executing command ordered by ExecutionKeyword >>", "info"), b.stop(), c.recognizing = !1, i.wildcard ? i.objeto.action(i.indice, i.wildcard.item, i.wildcard.full) : i.objeto.action(i.indice);
                                    break
                                }
                            }
                    ...
                    for (var e = a.resultIndex; e < d; ++e) {
                        ...
                        if (a.results[e].isFinal) {
                            var i = q(h.trim());
                            if ("function" == typeof c.helpers.redirectRecognizedTextOutput && c.helpers.redirectRecognizedTextOutput(h, !1), i !== !1 && 1 == c.recognizing) {
                                f.debug("<< Executing Matching Recognition in quick mode >>", "info"), b.stop(), c.recognizing = !1, i.wildcard ? i.objeto.action(i.indice, i.wildcard.item) : i.objeto.action(i.indice);
                                break
                            }
                        } else {
                            var i = q(h.trim());
                    ...
                    if (m(g.TEXT_RECOGNIZED), "function" != typeof c.helpers.remoteProcessorHandler) return f.debug("The remoteProcessorService is undefined.", "warn");
                    ...
                    if (c.obeying) j(a);
                        ...
                        for (var b = "", d = "", e = 0; e < a.results.length; ++e) a.results[e].final ? b += a.results[e][0].transcript : d += a.results[e][0].transcript;
        ...
        f.simulateInstruction = function (a) {
            ...
            var b = q(a);
            return "object" != typeof b ? (console.warn("No command founded trying with " + a), !1) : b.objeto ? (b.objeto.smart ? (f.debug("Smart command matches with simulation, executing", "info"), b.objeto.action(b.indice, b.wildcard.item, b.wildcard.full)) : (f.debug("Command matches with simulation, executing", "info"), b.objeto.action(b.indice)), !0) : void 0
        };
        var q = function (a) {
            ...
            for (var b = 0; b < k.length; b++) {
                for (var d = k[b], e = d.indexes, h = -1, i = 0; i < e.length; i++) {
                    var j = e[i];
                    if (d.smart) {
                        if (j instanceof RegExp) j.test(a) && (f.debug(">> REGEX " + j.toString() + " MATCHED AGAINST " + a + " WITH INDEX " + i + " IN COMMAND ", "info"), h = parseInt(i));
                        else if (j.indexOf("*") != -1) {
                            var n = j.split("*");
                            if (n.length > 2) {
                                console.warn("Artyom found a smart command with " + (n.length - 1) + " wildcards. Artyom only support 1 wildcard for each command. Sorry");
                                continue
                            }
                            var o = n[0],
                                p = n[1];
                            ...
                            } else if (!(a.indexOf(o) == -1 && a.toLowerCase().indexOf(o.toLowerCase()) == -1 || a.indexOf(p) == -1 && a.toLowerCase().indexOf(p.toLowerCase()) == -1)) {
                                var q = a.replace(o, "").replace(p, "");
                                q = q.toLowerCase().replace(o.toLowerCase(), "").replace(p.toLowerCase(), ""), q = q.toLowerCase().replace(p.toLowerCase(), ""), h = parseInt(i)
                            }
                ...
                for (var d = k[b], e = d.indexes, h = -1, i = 0; i < e.length; i++) {
                    ...
                    if (!d.smart) {
                        ...
                        if (a.toLowerCase() === j.toLowerCase()) {
                            f.debug(">> MATCHED OPTION CHANGING ALL TO LOWERCASE " + j + " AGAINST " + a + " WITH INDEX " + i + " IN COMMAND ", "info"), h = parseInt(i);
                            break
                        }
                    }
                }
                ...
                for (var d = k[b], e = d.indexes, h = -1, i = 0; i < e.length; i++)
                    if (!d.smart) {
                        ...
                        if (a.toLowerCase().indexOf(j.toLowerCase()) >= 0) {
                            f.debug(">> MATCHED INDEX OPTION CHANGING ALL TO LOWERCASE " + j + " AGAINST " + a + " WITH INDEX " + i + " IN COMMAND ", "info"), h = parseInt(i);
                            break
                        }
                    } if (h >= 0) return m(g.COMMAND_MATCHED), {
                    indice: h,
            ...
            if (c.soundex)
                for (var b = 0; b < k.length; b++)
                    for (var d = k[b], e = d.indexes, h = -1, i = 0; i < e.length; i++) {
                        var j = e[i];
                        if (!d.smart && l.soundex(a) == l.soundex(j)) return f.debug(">> Matched Soundex command '" + j + "' AGAINST '" + a + "' with index " + i, "info"), h = parseInt(i), m(g.COMMAND_MATCHED), {
                            indice: h,
                            objeto: d
                        }
                    }
        ...
        return f.debug = function (a, b) {
            ...
            if ("file:" == a.location.protocol) {
                var b = "Fatal Error Detected : It seems you're running the artyom demo from a local file ! The SpeechRecognitionAPI Needs to be hosted someway (server as http or https). Artyom will NOT work here, Sorry.";
            ...
            if (!f.device.isChrome) {
                var b = "Fatal Error Detected: You are not running Google Chrome ! SpeechRecognitionAPI and SpeechSynthesisAPI is only available in google chrome ! ";
                return console.error(b), {
                    code: "artyom_error_browser_unsupported",
                    message: b
                }
            }
            return "https:" != a.location.protocol && console.warn("Artyom is not running in HTTPS protocol,running in protocol : " + a.location.protocol + " that means the browser will ask the permission of microphone too often. You need a HTTPS Connection if you want artyom in continuous mode !"), !1
        }, f.redirectRecognizedTextOutput = function (a) {
            return "function" != typeof a ? (console.warn("Expected function to handle the recognized text ..."), !1) : (c.helpers.redirectRecognizedTextOutput = a, !0)
            ...
            return b.continuous = !0, b.interimResults = !0, b.lang = c.lang, b.onresult = function (b) {
                for (var c = "", d = "", e = 0; e < b.results.length; ++e) b.results[e].final ? c += b.results[e][0].transcript : d += b.results[e][0].transcript;
